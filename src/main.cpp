...
void Alarme(){
  ...
  //Unix actual timestamp is more than 5 minutes lower than time set.
    ...
    uint8_t red = map((WakeTime - time), ecartFromHigh, 0, 0, AlarmeRed);
    uint8_t green = map((WakeTime - time), ecartFromHigh, 0, 0, AlarmeGreen);
    uint8_t blue = map((WakeTime - time), ecartFromHigh, 0, 0, AlarmeBlue);
...
void displayColors(int dataSmartEcl[],uint8_t longueur, int cooldown, String mode){
  ...
  if((sunPosition(dataSmartEcl, 3) == "soir") && (mode = "SmartLight")){ //If we are in the evening slice
    ...
    int colorValArray[3] = {rouge, vert, bleu}; //int color array
    String colorNameArray = "rgb"; //string that contains the r, g and b key words
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length){
  ...
  if(type == WStype_TEXT){
    ...
    if(payload[0] == 'R'){
      ...
      //NRF24 transmit
      String redStr = String(strCouleur[0]);
      redStr += couleur; //String construction of the word to send

      uint8_t data[sizeof(redStr)];
      for(int i = 0;i<sizeof(redStr);i++){
        data[i]=uint8_t(redStr[i]); //converter from char (ex:"r345") to unsigned int
      }
      //data sent
      nrf24.send(data, sizeof(data));
      nrf24.waitPacketSent();
    ...
    if(payload[0] == 'G'){
      ...
      //NRF24 transmit
      String greenStr = String(strCouleur[1]);
      greenStr += couleur; //String construction of the word to send

      uint8_t data[sizeof(greenStr)];
      for(int i = 0;i<sizeof(greenStr);i++){
        data[i]=uint8_t(greenStr[i]);
      }
      //data sent
      nrf24.send(data, sizeof(data));
      nrf24.waitPacketSent();//converter from char (ex:"r345") to unsigned int
    ...
    if(payload[0] == 'B'){
      ...
      //NRF24 transmit
      String blueStr = String(strCouleur[2]);
      blueStr += couleur; //String construction of the word to send

      uint8_t data[sizeof(blueStr)];
      for(int i = 0;i<sizeof(blueStr);i++){
        data[i]=uint8_t(blueStr[i]); //converter from char (ex:"r345") to unsigned int
      }
      //data sent
      nrf24.send(data, sizeof(data));
      nrf24.waitPacketSent();
